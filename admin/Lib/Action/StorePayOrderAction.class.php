...
class StorePayOrderAction extends CommonAction{
	...
	public function view_order()
	{
	    ...
	    $payment_notice=M("PaymentNotice")->where("order_type=4 and is_paid=1 and order_id=".$storePayOrder['id'])->findAll();
		$payment_arr=array();
	    foreach($payment_notice as $k=>$v){
	    	$payment_notice[$k]['payment_name']=M("Payment")->where("id=".$v['payment_id'])->getField("name");
	    	$payment_str.=$payment_notice[$k]['payment_name'].":".format_price($payment_notice[$k]['money'])."  ";
	    }
	    $storePayOrder['payment_name']=$payment_str;
