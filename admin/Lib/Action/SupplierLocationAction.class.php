	...
	public function update() {
		...
		if (false !== $list) {
		   //DC
			...
			if($data['is_main']==1)
			{
				M(MODULE_NAME)->where("supplier_id=".$data['supplier_id']." and id <> ".intval($_REQUEST['id']))->setField("is_main",0);
			}
			if(M(MODULE_NAME)->where("supplier_id=".$data['supplier_id']." and is_main = 1")->count()==0)
			{
				M(MODULE_NAME)->where("id=".intval($_REQUEST['id']))->setField("is_main",1);
			}
