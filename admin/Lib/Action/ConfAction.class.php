...
class ConfAction extends CommonAction{
	...
	public function update()
	{
	   ...
	   if(mb_strlen($_REQUEST['COUPON_NAME'],'UTF8')>4){
	       $this->error ('消费券名称不可超过四个字');
	   }
	...
	{
		$conf_res = M("MConfig")->where("code!='close_index_cate'")->order("group_name asc,sort asc")->findAll();
		$wx_appid='';
		$wx_secrit='';
		$wx_url='';
		...
		foreach($conf_res as $k=>$v)
		{
			...
			//$v['value_scope'] = explode(",",$v['value_scope']);
			if($v['code']!='index_logo' && $v['code']!='region_version' && $v['code']!='program_title' )
	...
	public function savemobile()
	{
		...
		{
			M("MConfig")->where("code='".$k."' and code!='close_index_cate'")->setField("val",$v);
		}
		$this->success("保存成功");
	}
