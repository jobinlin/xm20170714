...
class EcvTypeAction extends CommonAction{
	...
	public function insert() {
		...
		if(intval($data['money'])>999 || intval($data['money'])<1)
		{
		    $this->error(L("请输入1~999之间的数字！"));
		}
		if($data['total_limit'] && intval($data['total_limit'])<0)
		{
		    $this->error(L("发放总量不能为负数"));
		}
		if($data['expire_day'] && intval($data['expire_day'])<0)
		{
		    $this->error(L("有效天数不能为负数"));
		}
		if($data['start_use_price'] && intval($data['start_use_price'])<0)
		{
		    $this->error(L("使用限制不能为负数"));
		}
		if($data['exchange_score'] && intval($data['exchange_score'])<0)
		{
		    $this->error(L("兑换积分不能为负数"));
		}
		if($_REQUEST['exchange_limit_score'] && intval($_REQUEST['exchange_limit_score'])<0)
		{
		    $this->error(L("会员兑换量不能为负数"));
		}
		if($_REQUEST['exchange_limit_bonus'] && intval($_REQUEST['exchange_limit_bonus'])<0)
		{
		    $this->error(L("会员兑换量不能为负数"));
		}

		if($_REQUEST['valid_type']==1){
		    $data['begin_time']=0;
		    $data['end_time']=0;
		}else{
		    $data['validity_day']=0;
	...
	public function edit() {		
		...
		$vo['money'] = round($vo['money'],2);
		$vo['start_use_price'] = round($vo['start_use_price'],2);
		$this->assign ( 'vo', $vo );
		$this->assign('tpls',$this->load_tpl_file());
		$this->display ();
	}
	...
	public function doSend()
	{
		...
		//判断是否过期
		$map['id']=$ecv_type_id;
		$ecv=M("ecv_type")->where($map)->find();
		if($ecv['end_time']!=0 && $ecv['end_time']<NOW_TIME)
		{
