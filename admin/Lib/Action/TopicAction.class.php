				...
				if ($list!==false) {
					...
					foreach($rel_data as $topic)
					{
						...
						if(intval(app_conf('USER_DELETE_MONEY'))<0 || intval(app_conf('USER_DELETE_POINT'))<0 || intval(app_conf('USER_DELETE_SCORE'))<0)
						{
							...
							modify_account(array("money"=>intval(app_conf('USER_DELETE_MONEY')),"is_admin"=>1,"score"=>intval(app_conf('USER_DELETE_SCORE')),"point"=>intval(app_conf('USER_DELETE_POINT'))),$topic['user_id'],"主题被删除");
						}
						if($topic['fav_id']>0)
						{
...
//					}
	...
	public function toogle_status()
	{
		...
		$title=M(MODULE_NAME)->where("id=".$id)->getField("title");
		...
		if($field == 'is_top' || $field == 'is_best' || $field == 'is_recommend')
		{
			if(!$c_is_effect)
			{
				...
				if($field == 'is_top'){
				    $log_txt = '管理员操作置顶';
				    $content="您的".$title."分享被置顶";
				    if ($score){
				        $content.="，获得积分".$score;
				    }
				    $msgClass->send_msg($user_id,$content,"notify",array("type"=>4));
				}
				if($field == 'is_best'){
				    $log_txt = '管理员操作精华';
				}
				if($field == 'is_recommend'){
				    $log_txt = '管理员操作推荐';
				    $content="您的".$title."分享被推荐";
				    if ($score){
				        $content.="，获得积分".$score;
				    }
				    $msgClass->send_msg($user_id,$content,"notify",array("type"=>3));
				}
			}
			else
			{
