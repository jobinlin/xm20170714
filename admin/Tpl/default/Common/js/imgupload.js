$(document).ready(function(){
	...
	max_img_size = max_img_size + 1;
	if($(".img-show-box .img-item").length==max_img_size){
	...
	$(".img-show-box .img-item .item_span").live('click',function(){
		...
		// var imgstyle = $(this).find('img').attr('style');
		// $('.demo_img_attr').find('img').attr('src', imgsrc);
		// $('.demo_img_attr').find('img').attr('style', imgstyle);
		// $('.big_demo_img').show();
		// $('.img_bg_box').show();
	});
...
function upload_img(img_index){
	 ...
	 var url = ROOT+"?"+VAR_MODULE+"=Public&"+VAR_ACTION+"=upload_img";
	 $.ajax({
		url:url,
		data:query,
		type:"post",
		...
		success:function(data){
			...
			if(data.status==1){
				...
				var style="background-image:url('"+data.file_path+"');background-size: cover;background-position: 50% 20%;background-repeat: no-repeat;"
				$(".img-show-box .img-index-"+img_index+" .item_span").attr('style',style);

				// var imgHtml = '<img src="'+data.file_path+'" style="width:100%;height:100%">'
				// $(imgHtml).appendTo($(".img-show-box .img-index-"+img_index+" .item_span"));
			}else{
				alert(data.info);
			}
