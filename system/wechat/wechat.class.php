...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @return boolean
	 */
	protected function set_php_file($filename, $value){
		$data = '';
	    $data->expire_time = NOW_TIME + 3600;
	    $data->access_token = $value;
	    $data = json_encode($data);
	    
	    try {
    	    $fp = fopen($filename, "w");
    	    fwrite($fp, "<?php exit();?>" . $data);
    	    fclose($fp);
    	    return true;
	    }catch (Exception $e) {   
	        logger::write('system/wechat/wechat类access_token缓存失败:'.$e->getMessage());
	    }
	}
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		// 获取缓存的access_token
		$data = json_decode($this->get_php_file(APP_ROOT_PATH."/public/access_token.php"));
		if ($data->expire_time > NOW_TIME && $data->access_token != '' ) {
		...
		if ($result)
		{
			...
			$this->set_php_file(APP_ROOT_PATH."/public/access_token.php", $this->access_token);
			return $this->access_token;
		}
		return false;
	}
	 ...
	 * 微信获取带参数的二维码（永久的）
	 */
	public function newGetQrCode($scene_str){
	    ...
	    $ticket_info = json_decode($this->http_post($url,$param));
	    
	    $get_img_url = "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket={$ticket_info->ticket}";
	    
	    return $this->http_get($get_img_url);
	}
