...
function do_login_biz($account_user,$account_password){
	$biz_data = $GLOBALS['db']->getRow("SELECT sa.* FROM ".DB_PREFIX."supplier_account as sa left join ".DB_PREFIX."supplier as s on sa.supplier_id=s.id WHERE sa.account_name='".$account_user."' AND sa.is_delete = 0");
	...
	if(!$biz_data)
	{
	    ...
	    $account_locations = $GLOBALS['db']->getAll("select sl.location_id from ".DB_PREFIX."supplier_account_location_link as sl left join ".DB_PREFIX."supplier_location as l on l.id=sl.location_id where l.is_effect=1 and sl.account_id = ".$biz_data['id']);
