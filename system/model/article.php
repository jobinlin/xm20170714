...
function load_article($type , $limit=10){
   $article_list = $GLOBALS['db']->getAll("select a.* from ".DB_PREFIX."article as a left join ".DB_PREFIX."article_cate as ac on a.cate_id=ac.id where ac.type_id=".intval($type)." limit ".$limit); 
   return $article_list;
}
?>