...
// 		//商户信息
		...
		//开始输出库存json
		$attr_stock_list =$GLOBALS['db']->getAll("select id,attr_cfg,stock_cfg,attr_str,buy_count,attr_key,price,add_balance_price from ".DB_PREFIX."attr_stock where deal_id = ".$deal['id'],false);
		$deal_attr_price_all=$GLOBALS['db']->getAll("select id,price from ".DB_PREFIX."deal_attr where deal_id = ".$deal['id']);
		foreach($deal_attr_price_all as $k=>$v)
		{
			$deal_attr_price[$v['id']] = $v['price'];
		}
