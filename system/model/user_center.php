 ...
 * @return array          
 */
function getUserMsg($limit, $user_id, $msg_type = 'notify')
{
	
	$typeCond = ' AND type = "'.$msg_type. '"';
	$sql = 'SELECT * FROM '.DB_PREFIX.'msg_box where user_id='.intval($user_id).' and is_delete = 0'.$typeCond.' ORDER BY id desc LIMIT '.$limit;
	$countSql = 'SELECT count(*) FROM '.DB_PREFIX.'msg_box where user_id='.intval($user_id).' and is_delete = 0'.$typeCond;
