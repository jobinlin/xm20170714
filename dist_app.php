...
if($act=="init")
{
	...
	$root['wx_app_api'] = 0;
	if(strim($m_config['wx_app_key'])!=""&&strim($m_config['wx_app_secret'])!="")
	{
		$root['wx_app_api'] = 1;
	}
	...
	$root['statusbar_color'] = '#e6e6e6';//状态栏,颜色
	$root['topnav_color'] = '#d82020';//顶部导航栏,颜色
	
	$root['ad_img'] = '';//启动时的广告图
	$root['ad_http'] = '';//启动时的广告连接内容
	$root['ad_open'] = 0;//点击广告内容，打开方式：0:在第一个webveiw中打开;1:新建一个webview打开连接
	
	$root['site_url'] = SITE_DOMAIN.APP_ROOT."/wap/dist.php?show_prog=1";//h5首页地址
...
function app_version($dev_type,$version)
{
	...
	$root = array();
	if ($dev_type == 'android'){
		$root['serverVersion'] = $m_config['android_biz_version'];//android版本号
		if ($version < $root['serverVersion']){

			$root['filename'] = $site_url.$m_config['android_biz_filename'];//android下载包名
			$root['android_upgrade'] = $m_config['android_biz_upgrade'];//android版本升级内容
			if(file_exists(APP_ROOT_PATH.$m_config['android_biz_filename']))
			{
				$root['hasfile'] = 1;
				$root['filesize'] = filesize(APP_ROOT_PATH.$m_config['android_biz_filename']);
				$root['has_upgrade'] = 1;//1:可升级;0:不可升级
				$root['forced_upgrade'] = intval($m_config['android_biz_forced_upgrade']);//0:非强制升级;1:强制升级
			}
			else
			{
				$root['hasfile'] = 0;
		...
		$root['serverVersion'] = $m_config['ios_biz_version'];//ios版本号

		if ($version < $root['serverVersion']){
			$root['ios_down_url'] = $m_config['ios_biz_down_url'];//ios下载地址
			$root['ios_upgrade'] = $m_config['ios_biz_upgrade'];//ios版本升级内容
			$root['has_upgrade'] = 1;//1:可升级;0:不可升级
			$root['forced_upgrade'] = intval($m_config['ios_biz_forced_upgrade']);//0:非强制升级;1:强制升级
		}else{
			$root['has_upgrade'] = 0;//1:可升级;0:不可升级
		}
	}else{
