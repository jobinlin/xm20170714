...
function suiShow($msg, $jump = '', $delay = 2000)
{
	if (empty($msg) || !is_string($msg)) {
		$msg = '正在跳转...';
	}
	$script = 'var suijump = function(){$.toast("'.$msg.'");';
    if($jump!=''){
        $script .= 'setTimeout(function(){$.router.load("'.$jump.'", true)},'.$delay.')';
    }
	$script .= '}';
	return $script;
}
...
function login_is_app_jump(){
	if(APP_INDEX=='app'){
		echo "<script>App.login_sdk();</script>";exit;
	}else{
