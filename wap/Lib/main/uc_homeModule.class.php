...
class uc_homeModule extends MainBaseModule
{
	...
	public function do_fav_topic()
	{
	    ...
	    $data = call_api_core("uc_home","wap_do_fav",$param);
	...
	public function cancel_fav()
	{
	    ...
	    $data = call_api_core("uc_home","wap_cancel_fav",$param);
	...
	public function do_reply(){
	    ...
	    if($data['user_login_status']!=LOGIN_STATUS_LOGINED){
	        ...
	        $data['reply_html'] = '<li class="r-item r_sub_data_id_'.$reply_data['reply_id'].' data-id="'.$reply_data['reply_id'].'"><a class="name_link" href="'.$u_url.'">'.$reply_data['user_name'].'</a>：<div class="r-con"  onclick="_reply_click('.$reply_data['reply_id'].', event);">'.$reply_data['content'].'</div></li>';
	    }
	...
	public function load_move_reply(){
	    ...
	    if($data['reply_data']){
            ...
            foreach ($data['reply_data'] as $k=>$v){
                ...
                $data['reply_html'] .= '<li class="r-item r_sub_data_id_'.$v['id'].'" data-id="'.$v['id'].'">'.
	            '<a class="name_link" href="'.$u_url.'">'.$v['user_name'].'</a>：'.
	            '<div class="r-con"  onclick="_reply_click('.$v['id'].', event)" data-id="'.$v['id'].'">'.$v['content'].'</div>'.
	            '</li>';
            }
	...
	public function downTopic()
	{
		...
		$data = call_api_core("uc_home","downTopic",$param);
		if($data['user_login_status']!=LOGIN_STATUS_LOGINED) {
	        $ajax_data['status'] = -1;
	        $ajax_data['info'] = "请先登录后操作";
	        $ajax_data['jump'] = wap_url("index","user#login");
			...
			if ($data['data_list']) {
				...
				$ajax_data['status'] = 1;
				$GLOBALS['tmpl']->assign('data_list', $data['data_list']);
				$ajax_data['html'] = $GLOBALS['tmpl']->fetch('uc_home_topic.html');
			}
		}
		ajax_return($ajax_data);
	}
