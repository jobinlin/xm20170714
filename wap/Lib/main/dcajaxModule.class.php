...
class dcajaxModule extends MainBaseModule
{
	 ...
	 */
	public function old_dc_add_cart(){
	...
	public function dc_add_cart()
	{
		...
		$param = array(
			'location_id' => intval($_REQUEST['location_id']),
			'menu_id' => intval($_REQUEST['menu_id']),
			'number' => intval($_REQUEST['number_total']),
		);
		if (isset($_REQUEST['table_menu_id'])) {
			$param['table_menu_id'] = intval($_REQUEST['table_menu_id']);
		}
		$data = call_api_core('dcajax', 'dc_add_cart', $param);
		ajax_return($data);
	}

	/**
	 * 获取购物车的的信息，订座页面刷新点菜部分用
	 * @return json 
	 */
	public function dc_res_cart_list()
	{
		...
		$param = array(
			'location_id' => intval($_REQUEST['location_id']),
			'table_menu_id' => intval($_REQUEST['table_menu_id']),
		);
		$data = call_api_core('dcajax', 'dc_res_cart_list', $param);
		ajax_return($data);
	}
	 ...
	 * 清空外卖购物车
	 */
	public function old_dc_cart_clear(){
		global_run();
		$location_id=intval($_REQUEST['location_id']);
		$user_id=isset($GLOBALS['user_info']['id'])?$GLOBALS['user_info']['id']:0;
	...
	public function dc_cart_clear()
	{
		...
		$param['location_id'] = intval($_REQUEST['location_id']);
		if (isset($_REQUEST['table_menu_id'])) {
			$param['tid'] = intval($_REQUEST['table_menu_id']);
		}
		$data = call_api_core('dcajax', 'dc_cart_clear', $param);
		ajax_return($data);
	}
	...
	public function get_dc_cart_list()
	{
		global_run();
		$location_id = intval($_REQUEST['location_id']);
		$result = call_api_core('dcajax', 'get_dc_cart_list', array('location_id' => $location_id));
		ajax_return($result);
	}
