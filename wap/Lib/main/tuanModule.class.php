...
class tuanModule extends MainBaseModule
{
    public function index()
    {
        ...
        $param['order_type'] = strim($_REQUEST['order_type'])?strim($_REQUEST['order_type']):'avg_point';


        $request = $param;
        $data = call_api_core("tuan","index_v2",$param);
        ...
        foreach($tuan_list as $k=>$v){
            $tuan_list[$k]['count'] = count($v['deal'])-2;
            $distance = $v['distance'];
            $distance_str = "";
            if($distance>0)
            {
            ...
            foreach ($v['deal'] as $key=>$value){

                $tuan_list[$k]['deal'][$key]['current_price'] = format_price_html($value['current_price']);
                $tuan_list[$k]['deal'][$key]['origin_price'] = round($value['origin_price'],2);
                if (is_array($v)){
                    $tuan_list[$k]['deal'][$key]["url"] = wap_url("index","deal", array('data_id'=>$value['id']));
                }
        ...
        $GLOBALS['tmpl']->assign("sort_url",wap_url("index","tuan",$sort_param));

        $distance_param = $param;
        $distance_param['order_type'] = "distance";
        $GLOBALS['tmpl']->assign("distance_url",wap_url("index","tuan",$distance_param));
        $GLOBALS['tmpl']->assign("param",$param);
// 		$back_url = wap_url("index","main");
