...
/**
 * 公共的分页方法
 * @param  int $total    总的数据数量
 * @param  int $size  每页显示的数量 如不传值默认为 PAGE_SIZE
 * @return null        
 */
function formatPage($total, $size = '')
{
	...
	$page = new Page($total,$size);   //初始化分页对象
    $p  =  $page->show();
    $GLOBALS['tmpl']->assign('pages',$p);
}
function formatLimit($page="",$size=""){
    if ($page == 0)
        $page = 1;
    if (empty($size)) {
        $size = PAGE_SIZE;
...
/**
 * 获取当前页面的标题
 * @param  string $ctl 模块名称
 * @param  string $act 方法名称
 * @return string      
 */
function getPageTitle($ctl='', $act='')
{
	if ($ctl == '') {
		$ctl = MODULE_NAME;
	}
	...
	$titleKey = strtolower($ctl).'_'.strtolower($act);
	$pageTitlesFile = dirname(__DIR__).'/Lang/zh-cn/'.APP_INDEX.'_title.php';
	
	if (is_file($pageTitlesFile) && is_readable($pageTitlesFile)) {
		$pageTitles = require($pageTitlesFile);
		if (array_key_exists($titleKey, $pageTitles)) {
			return $pageTitles[$titleKey];
		}
	}
	return '';
}
