...
class dealoModule extends BizBaseModule
{
	...
	{	
	    ...
	    $sql = "select distinct(doi.id),doi.*,do.memo,do.create_time,do.order_sn,do.order_status,do.ecv_money,do.youhui_money,do.total_price,do.pay_amount,doi.refund_status,do.invoice_info from ".$order_item_table_name." as doi left join ".
	 	    	$order_table_name." as do on doi.order_id = do.id left join ".
	    		DB_PREFIX."deal_location_link as l on doi.deal_id = l.deal_id ".	    		
	    		" where l.location_id in (".implode(",",$s_account_info['location_ids']).") and do.is_delete = 0 and do.type = 5 and doi.is_shop = 0 and do.pay_status = 2 $condition order by doi.id desc limit ".$limit;
	    
	    $sql_count = "select count(distinct(doi.id)) from ".$order_item_table_name." as doi left join ".
	    		$order_table_name." as do on doi.order_id = do.id left join ".
	    		DB_PREFIX."deal_location_link as l on doi.deal_id = l.deal_id ".
	    		" where l.location_id in (".implode(",",$s_account_info['location_ids']).") and do.is_delete = 0 and do.type = 5 and doi.is_shop = 0  and do.pay_status = 2 $condition ";
...
// 	    	$mobile.=$mobile_end;
	    	...
	    	$list[$k]['refund_status_2'] = $refund_status_2;
	    	
	    	$list[$k]['total_balance_price']=$v['balance_total_price']*($v['number']-$refund_status_2)/$v['number'];

	    	// 处理发票信息
    		if ($v['invoice_info']) {
    			$list[$k]['invoice_info'] = unserialize($v['invoice_info']);
    		}
	    }
	    ...
	    if ($invoiceConf && $invoiceConf['invoice_type']) {
	    	$GLOBALS['tmpl']->assign('hasInvoiceConf', 1);
	    }
