...
//		}
    ...
    function check_register_field($data)
    {
        ...
        {
            if($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."supplier_account where account_name = ".strim($data['account_name']))>0 ||
                $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."supplier_submit where is_publish <> 2 and account_name = ".strim($data['account_name']))>0){
                $data['status'] = false;
