...
class withdrawalModule extends BizBaseModule
{
	...
	{		
		...
		$GLOBALS['tmpl']->assign("withdraw_money",$supplier_info['money']-$result);		
	...
	public function get_fx_withdraw_money(){
		if($GLOBALS["account_info"]['supplier_withdraw_cycle']>=0){
			$day=$GLOBALS["account_info"]['supplier_withdraw_cycle'];
		}else{
		...
		$withdraw_money=floatval($GLOBALS['db']->getOne("select sum(money) as withdraw_money from ".DB_PREFIX."supplier_statements where stat_time >= '".$withdraw_start_day ."' and stat_time<='".$withdraw_day."' and supplier_id=".intval($GLOBALS["account_info"]['supplier_id'])));
		//N天内不可提现金额
