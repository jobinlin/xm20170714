...
class dcModule extends BizBaseModule
{
	...
	public function dc_set()
	{
		...
		foreach ($delivery_data as &$deli) {
			$deli['start_price'] = round($deli['start_price'], 2);
			$deli['scale'] = round($deli['scale'], 2);
			$deli['delivery_price'] = round($deli['delivery_price'], 2);
		}unset($deli);
	    //获取打包费数据
	    $package_conf =  $GLOBALS['db']->getRow("select * from ".DB_PREFIX."dc_package_conf where location_id=".$data['id']);
	    $package_conf['package_start_price'] = round($package_conf['package_start_price'], 2);
	    $package_conf['package_price'] = round($package_conf['package_price'], 2);
	    ...
	    $GLOBALS['tmpl']->assign("is_open_dada_delivery", $account_info['is_open_dada_delivery']);
	    /* 系统默认 */
	...
	public function do_save_dcset(){
	    ...
	    $datasl['delivery_type']= intval($_REQUEST['delivery_type']);
	    ...
	    //打包费用
	    $package_conf['package_price'] = round(floatval($_REQUEST['package_price']),2);
	    $package_conf['package_start_price'] = round(floatval($_REQUEST['package_start_price']),2);
...
// 	            $GLOBALS['db']->autoExecute(DB_PREFIX."dc_supplier_location_open_time",$temp_time);
	    ...
	    foreach ($scale as $k=>$v){
	        if($v){
	            ...
	            $temp_delivery['scale'] = round(floatval($scale[$k]),2);
	            $temp_delivery['start_price'] = round(floatval($start_price[$k]),2);
	            $temp_delivery['delivery_price'] = round(floatval($delivery_price[$k]),2);
	...
	public function do_save_menu(){
	   ...
	   if($id>0){
	       ...
	       $GLOBALS['db']->query("delete from ".DB_PREFIX."dc_cart where menu_id=".$id);
	...
	public function dc_edit_rsitem(){
        ...
        // 项目备注字数统计
        $vo['comment_count'] = mb_strlen($vo['comment'], 'utf-8');
	...
	public function do_save_rsitem(){
	    ...
	    $data['is_effect'] = intval($_REQUEST['is_effect']);
	    $data['price'] = round(floatval($_REQUEST['price']),2);
	    $data['comment'] = mb_substr(strim($_REQUEST['comment']), 0, 15, 'utf-8');
