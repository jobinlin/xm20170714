...
class eventModule extends MainBaseModule
{
	...
	public function do_submit()
	{
		...
		else
		{
			...
			$count=$GLOBALS['db']->getRow("select submit_count,total_count from ".DB_PREFIX."event where id=".$event_id." and total_count > 0");
		    
		    if($count['submit_count']>=$count['total_count'])
			{
				$data['status'] = 0;
				$data['info'] = "活动名额已满";
				ajax_return($data);
