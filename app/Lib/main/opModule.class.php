...
class opModule extends MainBaseModule
{
		...
		if(check_user_auth("dp","setbest"))
		{
			...
			else
			{
				...
				if($change==1||$o_reason!=""){
					// 点评信息？？？
					send_msg_new($dp['user_id'], $reason, 'notify', array('type' => 3, 'data_id' => $dp['id']));
	...
	function op_dp_del($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","del"))
		{
			...
			if($change==1||$o_reason!="") {
				send_msg_new($dp['user_id'], $reason, "notify", array('type' => 20));
	...
	function op_dp_replydel($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","replydel"))
		{
			...
			if($change==1||$o_reason!="") {
				send_msg_new($dp_reply['user_id'], $reason, "notify", array('type' => 20));
	...
	function op_topic_del($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("topic","del"))
		{
			...
			if($change==1||$o_reason!="") {
				send_msg_new($topic['user_id'], $reason, "notify", array('type' => 20));
	...
	function op_topic_replydel($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($change==1||$o_reason!="") {
				send_msg_new($topic_reply['user_id'], $reason, "notify", array('type' => 20));
	...
	function op_group_setbest($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($topic['is_best'] == 0)
			{
				...
				if($change==1||$o_reason!="") {
					send_msg_new($topic['user_id'], $reason, 'notify', array('type' => 3, 'data_id' => $topic['id']));
	...
	function op_group_settop($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($topic['is_top'] == 0)
			{
				...
				if($change==1)
				{
					$account_change = get_op_change('group', 'settop');
					modify_account($account_change,$topic['user_id'],$reason);
					$reason .= get_op_change_show("group","settop");
				}
				if($change==1||$o_reason!="") {
					send_msg_new($topic['user_id'], $reason, "notify", array('type' => 4, 'data_id' => $topic['id']));
					// send_msg($topic['user_id'], $reason, "topic", $topic['id']);
				}
	...
	function op_msg_del($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($change==1||$o_reason!="") {
				send_msg_new($message['user_id'], $reason, "notify", array('type' => 20));
