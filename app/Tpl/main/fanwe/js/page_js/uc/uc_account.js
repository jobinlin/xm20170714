	...
	function init_bind_sms_btn() {
		if(!$(".setting_user_info").find("div.ph_verify_btn").attr("bindclick")) {
			...
			$(".setting_user_info").find("div.ph_verify_btn").bind("click", function() {
				init_bind_sms_btn_submit(this,0);
			});
		}
		function init_bind_sms_btn_submit(obj,is_confirm) {
			if($(obj).attr("rel") == "disabled")
				return false;
			var is_error = 0;
			var error_msg = '';
			...
			$.ajax({
				...
				success : function(data) {
					if(data.status) {
						if(data.status==2){
							$.showConfirm(data.info,function(){
								init_bind_sms_btn_submit(obj,1);
							});
						}else{
