...
function setPlace(myValue) {
	...
	function myFun() {
		try {
			...
			//判断是否存在商圈区域获取
			var district_obj=$("input[name='district']");
			if(district_obj.length > 0){
				var query = {ak:key,location:pp.lat+","+pp.lng,output:"json"};
				$.ajax({
					...
					success:function(obj){
						$("input[name='district']").attr('value',obj.result.addressComponent.district);
					}
				});
			}
		} catch (ex) {
...
function creat_map(xpoint, ypoint) {
	...
	marker_change.addEventListener("dragend", function(e) {
		if (confirm('是否更新标记坐标?')) {
			...
			var district_obj=$("input[name='district']");
			if(district_obj.length > 0){
				var query = {ak:key,location:e.point.lat+","+e.point.lng,output:"json"};
				$.ajax({
					url:"//api.map.baidu.com/geocoder/v2/",
					dataType:"jsonp",
					callback:"callback",
