$(function(){
	...
	$("select[name='online_check']").bind("change",function(){
		$("form[name='project_form']").submit();
	});
	...
	$("button.del_btn").bind("click",function(){
		...
		if(module_name=='youhui'){
			var show_html='<p style="text-align:center;">确定要删除该优惠吗？</p>';
		}else{
			var show_html='<p style="text-align:center;">--待审核数据将会永久删除--</p><p style="text-align:center;">--审核通过的仅删除申请记录--</p> <p style="text-align:center;">确定删除吗？</p>';
		}
		
		$.showConfirm(show_html,function(){
			$.ajax({ 
				url: ajax_url, 
				data: query,
				dataType: "json",
	...
	$(".is_effect").bind("click",function(){
		var status=$(this).attr("status");
		var id=$(this).attr("data-id");
		
		var query = new Object();
		query.act = "effect_change";
		query.id = id;
		...
		$.ajax({ 
			...
			success: function(obj){
				...
				{
					if(obj.is_effect)
						$(".is_effect[data-id='"+id+"']").html("有效");
					else
						$(".is_effect[data-id='"+id+"']").html("禁用");
				}else
				{
					$.showErr(obj.info);
				}
	...
	/*上架申请*/
	$("button.up_btn").bind("click",function(){
		var id = $(this).attr("data-id");
		
		if(id>0){
			$.showConfirm("决定好要上架吗?",function(){
				var query = new Object();
				query.act = "up_line";
				query.id = id;
				$.ajax({ 
