...
class cityApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		$root['type']=strim($GLOBALS['request']['type']);
	...
	public function searchcity(){
	    $keyword = strim($GLOBALS['request']['kw']);
	    if($keyword){
	    $sea_city = $GLOBALS['db']->getAll("select *  from ".DB_PREFIX."deal_city where pid>0 and is_effect = 1 and is_delete=0 and name like '%".$keyword."%'");
	    }
	    $city=array();
	    $city['list'] = $sea_city;
	    $city['count'] = intval(count($sea_city));
	    $root['city'] = $city;
	    return output($root);
	}
