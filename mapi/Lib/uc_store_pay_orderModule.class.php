...
class uc_store_pay_orderApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		else
		{
			...
			$sql = "select sto.* , sl.name as location_name from ".DB_PREFIX."store_pay_order as sto left join ".DB_PREFIX."supplier_location as sl on sto.location_id=sl.id where sto.user_id={$user_id} AND sto.is_delete=0 and pay_status = 2 order by sto.create_time desc limit ".$limit;
			$sql_count = "select count(*) from ".DB_PREFIX."store_pay_order as sto left join ".DB_PREFIX."supplier_location as sl on sto.location_id=sl.id where sto.user_id={$user_id} AND  sto.is_delete=0 and pay_status = 2";
			...
			foreach($list as $k=>$v)
			{
				...
				$order_item['exchange_money'] = round($v['exchange_money'],2);
				//开始处理订单状态
				$order_status = "";				
