...
class dcresApiModule extends MainBaseApiModule
{
	...
	{		
		...
		$adv_list = $GLOBALS['cache']->get("WAP_DCRES_ADVS_".intval($city_id));
		//广告列表
		if($adv_list===false)
		{
			...
			foreach($advs as $k=>$v)
			{
				...
				$adv_list[$k]['img'] =  get_abs_img_root(get_spec_image($v['img'],320,120,1)); //首页广告图片规格为 宽: 640px 高: 240px
				$adv_list[$k]['type'] = $v['type'];
				$adv_list[$k]['data'] = $v['data'] = unserialize($v['data']);
				$adv_list[$k]['ctl'] = $v['ctl'];
			}
			$GLOBALS['cache']->set("WAP_DCRES_ADVS_".intval($city_id),$adv_list,300);
		}
		if($deal_quan_id){
