...
class uc_reviewApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		if($user_login_status!=LOGIN_STATUS_LOGINED){
			...
			/*$user_info['uid'] = $user['id']?$user['id']:0;
			$user_info['user_name'] = $user['user_name']?$user['user_name']:0;
			$user_info['user_avatar'] = get_abs_img_root(get_muser_avatar($user['id'],"big"))?get_abs_img_root(get_muser_avatar($user['id'],"big")):"";
			...
			$isSupplier = intval($GLOBALS['request']['status']);
			$isS = ' and '. ($isSupplier ? ' deal_id = 0 and event_id = 0 and youhui_id = 0' : ' (deal_id != 0 or event_id != 0 or youhui_id != 0) ');

			$dp_res = get_dp_list($limit,""," user_id = ".$GLOBALS['user_info']['id'].$isS);
			...
			foreach($dp_list as $k=>$v) {
			    ...
			    if($v['deal_id']>0){
			        $data_info= $GLOBALS['db']->getRow("select d.name,d.icon,doi.attr_str from ".DB_PREFIX."deal as d left join ".DB_PREFIX."deal_order_item as doi on d.id=doi.deal_id where doi.order_id=".$v['order_id']." and d.id=".$v['deal_id']); 
			        if(!$data_info){
			            unset($dp_list[$k]);
			            continue;
			        }else{
                        $temp_arr['attr_str']=$data_info['attr_str'];
			            $temp_arr['type'] = "deal";
			            $temp_arr['data_id'] = $v['deal_id'];
			            $temp_arr['deal_icon'] = get_abs_img_root(get_spec_image($data_info['icon'],136,136,1));
			        }

			    }elseif($v['youhui_id']>0){
			        $data_info= $GLOBALS['db']->getRow("select name,icon from ".DB_PREFIX."youhui where id=".$v['youhui_id']);
			        if(!$data_info){
			            unset($dp_list[$k]);
			            continue;
			        }else{
			        ...
			        $data_info= $GLOBALS['db']->getRow("select icon,name from ".DB_PREFIX."event where id=".$v['event_id']);
			        if(!$data_info){
			            unset($dp_list[$k]);
			            continue;
			        }else{
			    ...
			    if($v['deal_id']==0&&$v['youhui_id']==0&&$v['event_id']==0){
			        $data_info = load_auto_cache("store",array("id"=>$v['supplier_location_id']));
			        $temp_arr['type'] = "store";
			        $temp_arr['data_id'] = $v['supplier_location_id'];
			        $temp_arr['deal_icon'] = get_abs_img_root(get_spec_image($data_info['preview'],136,136,1));
			    }

			    $temp_arr['user_name'] = $user['user_name'];
			    $temp_arr['content'] = $v['content'];
			    $temp_arr['create_time'] = $v['create_time'] > 0 ?to_date($v['create_time']):'';
			    $temp_arr['reply_time'] = $v['reply_time'] > 0 ?to_date($v['reply_time']):'';  
			    ...
			    if($v['images']){
			        foreach ($v['images'] as $ik=>$iv){
			            $ik=$ik+1;
			            $images[$ik]= get_abs_img_root(get_spec_image($iv,60,60,1));
			            $oimages[$ik] = get_abs_img_root($iv);
			        }
