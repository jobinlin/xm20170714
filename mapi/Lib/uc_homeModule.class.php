...
class uc_homeApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
        ...
        //返回当前页面的会员信息
        $home_user_data = array();
        $home_user_data['id'] = $home_user_info['id'];
        $home_user_data['user_name'] = $home_user_info['user_name'];
        $home_user_data['user_avatar'] = get_abs_img_root(get_muser_avatar($home_user_info['id'],"big"))?get_abs_img_root(get_muser_avatar($home_user_info['id'],"big")):"";
        $home_user_data['uc_home_bg'] = SITE_DOMAIN.APP_ROOT."/mapi/image/nouchomebg.jpg";
         
        $root['user_data'] = $home_user_data;

        // 返回当前用户的信息
        $root['user'] = array('id' => $user_id, 'user_name' => $user['user_name']);
...
//             $temp_data['images'] = $f_images;
            ...
            $type_txt = $v['type'] == 'share' ? '' : "分享了一个".$type_array[$v['type']];
            $temp_data['type_txt'] = $type_txt;
            ...
            $temp_data['show_time'] = format_show_date($v['create_time']);
            ...
            // 新增点赞数组
            $temp_data['fav_id_name'] = $v['fav_id_name'] ? unserialize($v['fav_id_name']) : array();
            if (array_key_exists($user_id, $temp_data['fav_id_name'])) {
                $temp_data['has_zan'] = 1;
            }
	...
	public function show()
	{
        ...
        $type_txt = $result['type'] == 'share' ? '' : "分享了一个".$type_array[$result['type']];
        $temp_data['type_txt'] = $type_txt;
        ...
        $temp_data['show_time'] = format_show_date($result['create_time']);
        $reply_list = get_topic_reply_list($result['id'],'0,'.PAGE_SIZE);
        $temp_data['reply_list'] = $reply_list?$reply_list:array();
        $temp_data['reply_is_move'] = count($reply_list) < PAGE_SIZE ? 0 : 1;//$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."topic_reply where topic_id=".$result['id'])>PAGE_SIZE?1:0;
        // 新增点赞数组
        $temp_data['fav_id_name'] = $result['fav_id_name'] ? unserialize($result['fav_id_name']) : array();
        if (array_key_exists($user_id, $temp_data['fav_id_name'])) {
            $temp_data['has_zan'] = 1;
        }
        ...
        if($user_login_status == LOGIN_STATUS_LOGINED) {
            ...
            if(!$topic) {
                ...
                // } else {
                    // 判断是否已经点赞 
                    // 写入点赞字段 数组序列化
                    $fav_ids = $GLOBALS['db']->getRow("select fav_id_name from ".DB_PREFIX."topic where id=".$id);
                    $fav_id_s = array();
                    if (!empty($fav_ids['fav_id_name'])) {
                        $fav_id_s = unserialize($fav_ids['fav_id_name']);
                        if (array_key_exists($user_id, $fav_id_s)) {
                            return output($root,0,$GLOBALS['lang']['TOPIC_FAVED']);
                        }
                    }
                    
                    $fav_id_s[$user_id] = $user['user_name'];
                    $fav_ids['fav_id_name'] = serialize($fav_id_s);
                    $result = $GLOBALS['db']->autoExecute(DB_PREFIX.'topic', $fav_ids, 'UPDATE', 'id='.$id);
                    if ($result) {
                        $do_fav = array(
                            'id' => $user_id,
     ...
     * @return array 
     */
    public function wap_cancel_fav()
    {
        $root = array();
        /*参数初始化*/
        $id = intval($GLOBALS['request']['id']); //主题ID
