	...
	public function index()
	{
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			$list = $GLOBALS['db']->getAll("select do.id ,do.order_sn ,do.location_id ,do.create_time ,do.order_status ,do.confirm_status ,do.pay_status ,do.total_price , do.pay_amount,do.payment_id ,do.refund_status,do.refuse_memo ,do.consignee ,do.order_delivery_time ,do.is_cancel ,do.rs_price ,do.is_rs ,
					do.is_dp ,do.location_name ,do.dp_id,do.is_dp ,do.ecv_money,do.promote_amount, sl.preview from ".DB_PREFIX."dc_order as do left join ".DB_PREFIX."supplier_location as sl on do.location_id=sl.id where do.is_rs=0 and do.user_id=".$user_id ." order by do.id desc limit ".$limit);
			
			foreach($list as $k=>$v)
			{
				unset($list[$k]['order_menu']);
				$list[$k]['menu_name'] = $GLOBALS['db']->getAll("select name from ".DB_PREFIX."dc_order_menu where order_sn = '".$v['order_sn']."'");
				$list[$k]['count'] = count($list[$k]['menu_name']);
				$list[$k]['preview']=get_abs_img_root(get_spec_image($v['preview'],360,270,1));
				$list[$k]['create_time_format']=to_date($v['create_time']);
				$list[$k]['total_price']=$v['total_price']-$v['ecv_money']-$v['promote_amount'];//实付金额为应付减去红包减去优惠
	...
	public function view()
	{
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			$order_info = $GLOBALS['db']->getRow("select id ,order_sn ,location_id ,create_time ,order_status ,confirm_status ,pay_status ,
					...
					location_name,dp_id,is_dp,dada_delivery,xpoint,ypoint  from ".DB_PREFIX."dc_order where type_del = 0 and user_id = ".$GLOBALS['user_info']['id']." and id = ".$id);
			$supplier_location_info=$GLOBALS['db']->getRow("select tel,preview from ".DB_PREFIX."supplier_location where id=".$order_info['location_id']);
			...
			$order_info['order_menu']=$GLOBALS['db']->getAll("select name ,num ,unit_price from ".DB_PREFIX."dc_order_menu where order_sn=".$order_info['order_sn']);
			...
			if($order_info['id']>0)
			{
				...
				if($order_info['order_delivery_time']==1){
					$order_info['order_delivery_time_format']='尽快配送';
				}else{
				...
				if(IS_OPEN_DADA){
				    ...
				    if($dada_delivery['order_status'] == 2 || $dada_delivery['order_status'] == 3 || $dada_delivery['order_status'] == 4){
				        ...
				        $order_info['transporterPhone'] = $dada_info['transporterPhone'];
				...
				$root['IS_OPEN_DADA'] = IS_OPEN_DADA;//是否开启达达配送
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			if($order_info)
			{
			    ...
			    if(NOW_TIME - $order_info['create_time']<$time){
			        return output($root,0,"下单后45分钟才可以催单哦");
			    }else{
			        
			        //催单时间，每次要间隔15分钟,以下SQL是查询有没有15分钟之内的催单记录
			        $dc_reminder=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."dc_reminder where order_id=".$id." and ".NOW_TIME." - create_time < 600 order by create_time desc limit 1");
			        if($dc_reminder){
			            //$dc_reminder存在，不可以催
			            	
			            return output($root,0,"亲,您催的太快了,10分钟催一次哦!");
			            ...
			            $biz_msg['content']="订单号".$order_info['order_sn']."，会员已催单，请尽快处理~";
			            $biz_msg['supplier_id'] = $order_info['supplier_id'];
			            $biz_msg['create_time']=NOW_TIME;
			            $biz_msg['type']='order';
			            $biz_msg['data']=serialize(array('id' => $order_info['id'], 'ctl' => 'biz_dc_order#view', 'title' =>22));
